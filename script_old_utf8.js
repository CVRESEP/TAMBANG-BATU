...
// ============================================================
// State Management ΓÇö Turso Integration
// ============================================================
...
async function fetchAllDataFromTurso() {
    console.log("≡ƒôÑ Mengambil data dari Turso via Cloudflare API...");
    try {
        ...
        saveData(_cache);
        console.log("Γ£à Data berhasil disinkronkan dari Turso.");
    } catch (e) {
        console.error("Γ¥î Gagal mengambil data dari Turso:", e);
    }
}
...
async function saveData(data, table = null, item = null) {
    ...
    if (table && item) {
        try {
            ...
            console.log(`Γ£à Berhasil simpan ke Turso (${table})`);
        } catch (e) {
            console.error(`Γ¥î Gagal simpan ke Turso (${table}):`, e);
...
async function deleteFromDatabase(table, id) {
    try {
        ...
        console.log(`Γ£à Berhasil hapus dari Turso (${table})`);
    } catch (e) {
        console.error(`Γ¥î Gagal hapus dari Turso (${table}):`, e);
    }
}
...
// App boot ΓÇö called once on page load
async function bootApp() {
    console.log("≡ƒ¢á∩╕Å [DEBUG] bootApp dipanggil (Mode Turso)!");
    ...
    try {
        ...
        console.log("Γ£à [DEBUG] App siap digunakan.");
    }
}
...
window.render_laporan = () => {
    ...
    window.generateLaporan = () => {
        ...
        registeredTypeIds.forEach(typeId => {
            ...
            // Always sum from expenseDetails ΓÇö works for both Operasional and Retribusi
            let total = 0;
            filtered.forEach(tx => {
                (tx.expenseDetails || []).forEach(d => {
...
async function executeBulkDelete(config, ids) {
    ...
    try {
        ...
        console.log(`Γ£à Berhasil hapus bulk dari Turso (${config.supabaseTable})`);
    } catch (e) {
        console.error(`Γ¥î Gagal hapus bulk dari Turso (${config.supabaseTable}):`, e);
